...
public class InspectionLabelingService {
	...
	/**
	 * 해당 담당자에게 작업 추가하기 한 가지 작업이라도 실패하면 rollback
	 * 
	 * @author 이동현
	 * @param updateTime
	...
	public String updateOvertime(UpdateTime updateTime) throws ParseException {
		...
		// 담당자의 당일 작업 개수 추가
		int overTimeResult = inspectionLabelingDao.updateLabelingWorkTime(updateTime.getReceiveItem(),
				updateTime.getReceiveQuantity(), updateTime.getLabelingWorkTimeNo());
		...
		int workTimeResult = inspectionLabelingDao.updateInspectionLabelingWork(lastStartDate, lastEndDate,
				updateTime.getLabelingWorkTimeNo(), updateTime.getPlacingOrderNo());
