...
public class ReleaseInpectionServiceImpl implements ReleaseInspectionService {
	...
	/**
	 * 출고검수/패킹 전체 건수 제외 모든 정보를 Mapper로부터 전달받음. 출고검수/패킹 전체 건수는 출고검수/패킹 진행단계 중 긴급, 일반
	 * 건수 각각을 합산해서 구함. DB사용 최소화 위함.
	 * 
	 * @author 최숙희
	 */
	@Override
	public Map<String, Object> getSummary() {
		...
		map.put("RLS_INSP_NUM",
				Integer.parseInt(map.get("EX_NUM").toString()) + Integer.parseInt(map.get("NM_NUM").toString()));
	...
	/**
	 * 1. afterPickingViewDao로부터 필터링된 목록의 전체 행수 받아와서 Pager객체 생성. 2.
	 * afterPickingViewDao로부터 페이지네이션 적용한 목록 받아옴. 3. 데이터 형식 조정.
	 * 
	 * @author 최숙희
	 */
	@Override
	public Map<String, Object> getAfterPickingList(AfterPickingFilter afterPickingFilter) {
		...
		List<HashMap<String, String>> ap = afterPickingViewDao.selectAll(afterPickingFilter, pager);

		// 비고란 합쳐서 가져온 값 할당.
		// 주문번호 중복없이 가져오기
		List<Long> ordNoList = afterPickingViewDao.selectOrderNo(afterPickingFilter, pager);
		// OI_NO 모으기.
		...
		for (int i = 0; i < ap.size(); i++) {
			...
			if (ap.get(i).get("ORD_NO") != null) {
				if (noteMap.get(String.valueOf(ap.get(i).get("ORD_NO"))) == null
					...
					map.replace("OI_NT", " "// 그리드 셀 병합을 위해, 메모값이 없는 경우 공백문자 할당.
					);
				} else {
					map.replace("OI_NT", noteMap.get(String.valueOf(ap.get(i).get("ORD_NO"))));
				}
			}

			// 출고요청서 출력일시 컬럼 값을 '담당자성명+\n+(+mm:dd+ +hh:MM+)'형태로 만듦.
			if (ap.get(i).get("RI_EMP_NAME") != null && ap.get(i).get("RI_RLS_PRT_DT") != null
					&& !ap.get(i).get("RI_RLS_PRT_DT").equals(" ")) {
				map.replace("RI_RLS_PRT_DT",
			...
			if (ap.get(i).get("RI_EMP_NAME") != null && ap.get(i).get("RI_RCPT_PRT_DT") != null
					&& !ap.get(i).get("RI_RCPT_PRT_DT").equals(" ")) {
				map.replace("RI_RCPT_PRT_DT",
						ap.get(i).get("RI_EMP_NAME") + "\n" + "("
			...
			if (ap.get(i).get("RI_RCPT_PRT_DT") != null && !ap.get(i).get("RI_RCPT_PRT_DT").equals(" ")) {
				map.replace("RI_DT", String.valueOf(ap.get(i).get("RI_RCPT_PRT_DT")).substring(5, 10) + " "
						+ String.valueOf(ap.get(i).get("RI_RCPT_PRT_DT")).substring(11, 16));
			}
